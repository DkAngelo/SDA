...
/** @brief La funzione `HeapMinInsertNode()` aggiunge un nodo a una coda di 
            priorità esistente, garantendo che le proprietà (min-)heap siano 
            rispettate.
...
/** @brief Dato un heap e l'indice di un nodo, la funzione `HeapMinMoveUp()` sposta
           il nodo verso l'alto, ovvero lo scambia con il nodo padre ricorsivamente
           fino a quando le condizione (min-)heap non sono rispettate.
...
/** @brief Dato un heap e l'indice di un nodo, la funzione `HeapMinMoveDown()`
           sposta il nodo verso il basso, ovvero lo scambia con il minore dei 
           figli ricorsivamente fino a quando le condizione (min-)heap non sono
           rispettate.
