...
void StazioniServizioRec(double m, const double* d, const double* p, size_t n, double c, double s, bool* vcurr, bool* vbest, int i, double price, double* bestprice, int* cnt, int *ncall) {
	...
	vcurr[i] = 0;
	StazioniServizioRec(m, d, p, n, c, s, vcurr, vbest, i + 1, price, bestprice, cnt, ncall);
	vcurr[i] = 1;
	StazioniServizioRec(m, d, p, n, c, s, vcurr, vbest, i + 1, price, bestprice, cnt, ncall);
...
void StazioniServizio(double m, const double* d, const double* p, size_t n) {
	...
	int ncall = 0;
	int tick = clock();
	StazioniServizioRec(m, d, p, n, c, s, vcurr, vbest, 0, 0, &bestprice, &cnt, &ncall);
	int tock = clock();
